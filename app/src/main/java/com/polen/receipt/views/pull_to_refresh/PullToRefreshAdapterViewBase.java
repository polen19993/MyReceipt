...
public abstract class PullToRefreshAdapterViewBase<T extends AbsListView> extends PullToRefreshBase<T> implements OnScrollListener
{
	...
	public final void onScroll(final AbsListView view, final int firstVisibleItem, final int visibleItemCount, final int totalItemCount) {
		...
		if (DEBUG) {
			Utils.printDebug(null, "First Visible: " + firstVisibleItem + ". Visible Count: " + visibleItemCount + ". Total Items:" + totalItemCount);
		}
	...
	private void addIndicatorViews() {
		...
		if (mode.showHeaderLoadingLayout() && null == mIndicatorIvTop) {
			...
			params.rightMargin = getResources().getDimensionPixelSize(R.dimen.dip_10);
	...
	private boolean isFirstItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			if (DEBUG) {
				Utils.printDebug(null, "isFirstItemVisible. Empty View.");
			}
			return true;
		} else {
			/**
	...
	private boolean isLastItemVisible() {
		...
		if (null == adapter || adapter.isEmpty()) {
			if (DEBUG) {
				Utils.printDebug(null, "isLastItemVisible. Empty View.");
			}
			return true;
		} else {
			...
			if (DEBUG) {
				Utils.printDebug(null, "isLastItemVisible. Last Item Position: " + lastItemPosition + " Last Visible Pos: " + lastVisiblePosition);
			}
